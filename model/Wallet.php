...
class Wallet
{
    ...
    public function spendMoney(float $coin)
    {
        ...
        $sql =
            "UPDATE 
          `wallet` SET `coin`=:coin, updated_at=:updated_at WHERE user_id=:user_id";
        $params = [
            ':user_id' => $userIdentity['id'],
        ...
        $sql3 =
            "SELECT sum(`t`.`coin`) as `tr_coin`, `w`.`user_id`, `w`.`coin` FROM `wallet` as `w` INNER JOIN `transactions` as `t` ON `t`.user_id=`w`.`user_id` WHERE `w`.`user_id`=:user_id 
    ...
    {
        $sql = "
            SELECT `user_id`, `coin`, `created_at`, `updated_at` FROM `wallet` 
